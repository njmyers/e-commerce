...
export class Migration20210719001154 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table "address" ("id" serial primary key, "created_at" date not null, "address_line1" varchar(255) not null, "address_line2" varchar(255) null, "city" varchar(255) not null, "province" varchar(255) null, "postal_code" varchar(255) not null, "country" text check ("country" in (\'US\', \'ES\')) not null);');

    this.addSql('create table "user" ("id" serial primary key, "created_at" date not null, "updated_at" date not null, "email" varchar(255) not null, "name" varchar(255) not null, "role" text check ("role" in (\'admin\', \'merchant\', \'customer\')) not null, "password" varchar(255) null);');
    this.addSql('alter table "user" add constraint "user_email_unique" unique ("email");');
    this.addSql('create index "user_role_index" on "user" ("role");');

    this.addSql('create table "order" ("id" serial primary key, "created_at" date not null, "updated_at" date not null, "number" varchar(255) not null, "total" int4 not null, "tax" int4 not null, "notes" varchar(255) null, "shipped_at" date null, "customer_id" int4 not null);');
    this.addSql('alter table "order" add constraint "order_number_unique" unique ("number");');

    this.addSql('create table "shop" ("id" serial primary key, "created_at" date not null, "updated_at" date not null, "name" varchar(255) not null, "description" varchar(255) not null);');

    this.addSql('create table "product" ("id" serial primary key, "created_at" date not null, "updated_at" date not null, "name" varchar(255) not null, "description" varchar(255) not null, "price" int4 not null, "length" int4 not null, "height" int4 not null, "width" int4 not null, "mass" int4 not null, "shop_id" int4 not null);');

    this.addSql('create table "line_item" ("id" serial primary key, "created_at" date not null, "updated_at" date not null, "quantity" int4 not null, "product_id" int4 not null, "order_id" int4 not null);');
